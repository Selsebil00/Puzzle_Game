...
public class JigsawPuzzle : MonoBehaviour {
    ...
    public int placePrecision = 12;             // how precise must a piece beeing placed on the puzzle ( = percentage | 6 - 15 are good values , maybe higher up to 25 for small children )
	public bool scatterPieces = true;			// scatter pieces when puzzle starts - if false the pieces are placed on their spot on the puzzle    
	...
	protected void Update () {
        if (main == null)
        {
            ...
            if (!checkedOnce)
            {
                // check ONCE if PuzzleMain Script is found on JisawMain Prefab
                GameObject go = GameObject.Find("PuzzleMain");
                if (go == null)
                {
                    Debug.LogError("PuzzleMain (prefab) GameObject not added to scene!");
                    checkedOnce = true;
                    return;
                }
                // get PuzzleMain class for piece prototype access
                main = go.GetComponent("PuzzleMain") as PuzzleMain;
                // check if main is initialized correctly so isValid should be true
                if (main != null)
                    if (!main.isValid)
                    {
                        Debug.LogError("PuzzleMain (prefab) GameObject is not valid - Base puzzle pieces could not be found!");
                        main = null;
                        checkedOnce = true;
                        return;
                    }
        ...
        {
            // PuzzleMain was found and is valid so we can go on with our puzzle	
                    ...
                    {										
						...
						if (Input.touchCount==0 || (Input.touchCount>0 && dragTouch.fingerId == Input.touches[0].fingerId))
						{
	                        ...
	                        else
	                        {
                                ...
                                // cast a ray from the camera collect all hits that correspond with the 'puzzle' layer mask (31 = default but can be set on PuzzleMain class)
    ...
    private void SetSample()
    {
        ...
        // set image to puzzle material to sample image material (can be set on PuzzleMain class)
    ...
    private void InitPiece(GameObject puzzlePiece, Vector2 pos)
    {
        ...
        // set surface/image material to scatteredPieces material (can be set on PuzzleMainnClass)
        puzzlePiece.GetComponent<Renderer>().material = main.scatteredPieces;
        // set piece base material to pieceBase material (can be set on PuzzleMainClass)
